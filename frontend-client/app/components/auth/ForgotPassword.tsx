...
interface ResetPasswordFormProps {
  handleRestPassword: (payload: ResetPasswordPayload) => Promise<void>;
  ...
  handleSendRestPasswordCode: (payload: EmailPayload) => Promise<void>;
}
...
export const ResetPasswordForm: React.FC<ResetPasswordFormProps> = ({
  ...
  countdown,
  isSendingCode,
  handleSendRestPasswordCode,
}) => {
  const { language } = useLanguage();
  const getText = useGetText(language);
  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    handleRestPassword(formData);
  };
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-8">
      ...
      <div className="flex items-center space-x-2">
        ...
        <button
          type="button"
          onClick={() => handleSendRestPasswordCode({ email: formData.email })}
